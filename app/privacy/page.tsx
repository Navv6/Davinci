import Link from "next/link";

export const metadata = {
  title: "개인정보처리방침 | 다빈치노트",
};

const PRIVACY_TEXT = `[개인정보처리방침]

모듀라(이하 "회사")는 이용자의 개인정보를 중요하게 생각하며, 관련 법령을 준수합니다.

1. 수집하는 개인정보 항목
   회사는 다음과 같은 정보를 수집할 수 있습니다.

* 이메일, 로그인 정보 (회원가입 시)
* 서비스 이용 기록, 접속 로그, IP 주소
* 이용자가 입력한 텍스트 및 질의 데이터

2. 개인정보 수집 및 이용 목적
   회사는 다음의 목적을 위해 개인정보를 사용합니다.

* 서비스 제공 및 기능 개선
* 사용자 맞춤형 콘텐츠 제공
* 서비스 이용 분석 및 품질 향상

3. 개인정보 보유 및 이용 기간
   회사는 개인정보 수집 및 이용 목적이 달성된 후 해당 정보를 지체 없이 파기합니다.

다만, 서비스 운영 및 품질 개선을 위해 다음과 같이 일정 기간 보관할 수 있습니다.

* 서비스 이용 기록 및 접속 로그: 3개월
* 이용자가 입력한 데이터: 최대 30일 이내
* 회원 정보: 회원 탈퇴 시 즉시 삭제

관련 법령에 따라 보존이 필요한 경우 해당 기간 동안 보관합니다.

4. 개인정보의 제3자 제공
   회사는 원칙적으로 이용자의 개인정보를 외부에 제공하지 않습니다.
   다만, 서비스 제공을 위해 다음과 같은 외부 서비스를 이용할 수 있습니다.

* AI 서비스 제공자: OpenAI (openai.com)
  이 경우 필요한 최소한의 데이터(노드 텍스트, 주제)만 전송됩니다.

5. 개인정보 처리 위탁
   회사는 서비스 제공을 위해 일부 업무를 외부에 위탁할 수 있습니다.

* 인증 및 데이터베이스 서비스 제공자: Supabase (supabase.com)

6. 이용자의 권리
   이용자는 언제든지 자신의 개인정보를 조회, 수정, 삭제할 수 있습니다.

7. 개인정보 보호 조치
   회사는 개인정보 보호를 위해 기술적·관리적 조치를 취하고 있습니다.

8. 정책 변경
   본 방침은 관련 법령 및 회사 정책에 따라 변경될 수 있습니다.

문의: bwme43@gmail.com

시행일: 2026년 4월 6일`;

export default function PrivacyPage() {
  return (
    <main className="mx-auto max-w-2xl px-6 py-16 text-[#1a1208]">
      <Link
        href="/"
        className="mb-10 block text-[11px] italic tracking-[0.2em] text-[#8b6c42] hover:underline"
      >
        ← 다빈치 노트로 돌아가기
      </Link>
      <h1 className="font-display text-[2.5rem] font-light tracking-[0.04em]">
        개인정보처리방침
      </h1>
      <pre className="mt-8 whitespace-pre-wrap text-[13px] leading-7 tracking-[0.02em] text-[#3d2b12]">
        {PRIVACY_TEXT}
      </pre>
    </main>
  );
}
